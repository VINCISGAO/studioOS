...
export async function CreatorOnboardingPage({
  searchParams
}: {
  searchParams: SearchParams & { submitted?: string; error?: string };
}) {
  const locale = getLocale(searchParams);
  const t = copy[locale];
  const submittedId = typeof searchParams.submitted === "string" ? searchParams.submitted : null;
  const application = submittedId ? await getApplication(submittedId) : null;
  const error = searchParams.error === "missing" ? t.errorMissing : undefined;
                ...
                <CardContent className="p-6 sm:p-8">
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    ...
                    <Button asChild variant="outline">
                      <Link href={withLocale("/studio/onboarding", locale)}>{t.backForm}</Link>
                    </Button>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-6 sm:p-8">
                  <form action={submitOnboardingAction} className="grid gap-6">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <OnboardingField label={t.fields.studio_name} name="studio_name" required />
                      <OnboardingField label={t.fields.email} name="email" type="email" required />
                      <OnboardingCountryField label={t.fields.country} name="country" locale={locale} required />
                      <OnboardingField label={t.fields.portfolio_url} name="portfolio_url" type="url" required />
                      <OnboardingField
                        ...
                        required
                      />
                      <OnboardingField label={t.fields.tools} name="tools" placeholder={t.placeholders.tools} required />
                      <OnboardingField label={t.fields.base_price} name="base_price" type="number" min="1" required />
                      <OnboardingField
                        label={t.fields.delivery_speed}
                        name="delivery_speed"
                        placeholder={t.placeholders.delivery_speed}
                        required
                      />
                    </div>
...
function OnboardingCountryField({
  label,
  name,
  locale,
  required
  ...
  return (
    <div className="grid gap-2">
      ...
      >
        ...
        {getCountryOptions(locale).map(({ value, label: optionLabel }) => (

...
export function CinematicHeroStats({ locale }: { locale: Locale }) {
  ...
  return (
    ...
    >
      <div className="hero-stats-panel relative overflow-hidden rounded-xl border border-white/[0.1] bg-black/45 backdrop-blur-xl sm:rounded-[1.25rem]">
        <div className="hero-stats-shimmer pointer-events-none absolute inset-x-0 top-0 h-px" aria-hidden />

        <div className="relative grid grid-cols-2 gap-px bg-white/[0.08] lg:grid-cols-4">
          {stats.map((stat, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative bg-black/55 px-3 py-5 text-center sm:px-7 sm:py-9"
              >
                ...
                <p className="relative font-mono text-[1.45rem] font-semibold tracking-tight sm:text-[2.65rem]">
                  {meta.animate !== null ? (
                      <>
                        <AnimatedCounter value={meta.animate} locale={locale} />
                        {meta.suffix}
                        {"extra" in meta && meta.extra ? (
                          <span className="ml-0.5 text-lg text-emerald-400 sm:text-2xl">{meta.extra}</span>
                        ) : null}
                      </>
                    ) : (
                ...
                <p className="relative mt-2 text-[10px] leading-4 text-zinc-400 sm:text-sm">{stat.label}</p>
              </motion.div>
            );
          })}
        </div>

        <div className="relative border-t border-white/[0.08] px-4 py-6 sm:px-6 sm:py-8">
          <motion.p
            initial={reduce ? false : { opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, ease: cinematicEase, delay: 1.05 }}
            className="text-center text-[10px] font-medium uppercase tracking-[0.16em] text-zinc-500 sm:text-[11px] sm:tracking-[0.24em]"
          >
            {logos.label}
          </motion.p>
          <ul className="mx-auto mt-5 flex max-w-5xl flex-wrap items-center justify-center gap-x-5 gap-y-3 sm:mt-6 sm:gap-x-10">

...
**Golden baseline:** owner-locked **2026-07-11** · supersedes **2026-07-10** · 11-language hero videos (`cv=7`), **`HomeHeroVideo` player frozen**, mobile card nav, iPad/desktop responsive hero + CTA + cost table, hero subtitle「顶级广告不再是大公司专属」. See [`docs/HOMEPAGE_GOLDEN.md`](../docs/HOMEPAGE_GOLDEN.md) and [`docs/HOMEPAGE_HERO_VIDEO_BASELINE.json`](../docs/HOMEPAGE_HERO_VIDEO_BASELINE.json). Legacy anchors: `homepage-golden` branch · `homepage-v1` tag.

## Agent policy

**Owner lock (2026-07-11):** 没有项目 owner 在**当前对话**中的明确命令，**绝对不可以**改动首页栈。包括文案、视频 URL、**手机 / iPad / 电脑版布局**、间距、响应式断点、动画、资产、导航、以及 `components/marketing/home-hero-video.tsx`。

...
export function SelectionToolbar({
  ...
  const canSendToChat = Boolean(
    oneSelected && selected && resolveCanvasNodeChatReference(selected.data)
  );
    ...
    <div className="pointer-events-none absolute left-1/2 top-4 z-30 -translate-x-1/2">
      <div className="pointer-events-auto flex max-w-[calc(100vw-2rem)] items-center gap-0.5 overflow-x-auto rounded-xl border border-zinc-200 bg-white p-1 shadow-lg">
        ...
        <SelectionAction
          ...
          onClick={() => {
            if (!selected) return;
            const payload = resolveCanvasNodeChatReference(selected.data);
        ...
        {oneSelected && selected && canDownloadCanvasNode(selected.data) ? (
          <a

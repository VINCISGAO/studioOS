...
const copy = {
  en: {
    ...
    title: "Proposal Room",
    subtitle:
      "Professional communication on-platform. Contract auto-generated from agreement. Chat locks when production starts.",
    brand: "Brand",
    studio: "Studio",
...
export default async function ProposalRoomPage({ params, searchParams }: Props) {
  const [{ id }, query] = await Promise.all([params, searchParams]);
  const locale = getLocale(query);
  const t = copy[locale];

  const legacyDestination = await resolveLegacyProposalRedirectPath(id);
  if (legacyDestination) {
    permanentRedirect(withLocale(legacyDestination, locale));
  }

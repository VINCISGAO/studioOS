...
export class EscrowService {
  ...
  private async ensureEscrowRecord(campaignId: string) {
    ...
    const existing = await escrowRepository.findByCampaignId(campaignId);
  ...
  private async transitionEscrow(
    ...
    return runTransition({
      ...
      persist: async (next) => {
        await escrowRepository.updateStatus(campaignId, next as EscrowStatus);
      },
      domainEvent: {
  ...
  async startCheckout(campaignId: string, user: AuthUser & { email?: string }) {
    ...
    let escrow = await escrowRepository.findByCampaignId(campaignId);
    if (!escrow) throw appError("NOT_FOUND", "Escrow not found");
    ...
    if (escrow.status === EscrowState.CREATED) {
      ...
      escrow = await escrowRepository.findByCampaignId(campaignId);
    }
  ...
  async completePayment(input: {
    ...
    let escrow = await escrowRepository.findByCampaignId(input.campaignId);
    if (!escrow) {
    ...
    if (escrow.status === EscrowState.CREATED) {
      ...
      escrow = (await escrowRepository.findByCampaignId(input.campaignId))!;
    }
    ...
    if (input.stripePaymentId) {
      await escrowRepository.updateStatus(input.campaignId, EscrowState.HELD, {
        stripePaymentId: input.stripePaymentId
      });
    }

    const afterCreate = await prisma.campaign.findUniqueOrThrow({ where: { id: input.campaignId } });
    if ([CampaignState.ESCROW_PENDING, CampaignState.CREATOR_ACCEPTED].includes(afterCreate.status as typeof CampaignState.ESCROW_PENDING)) {
  ...
  async demoPay(campaignId: string, user: AuthUser) {
    ...
    if (!PAYABLE_CAMPAIGN_STATES.has(campaign.status)) {
      ...
      if (refreshed?.status === CampaignState.PRODUCING || refreshed?.status === CampaignState.ESCROW_FUNDED) {
        const escrow = await escrowRepository.findByCampaignId(campaignId);
        return {
          alreadyFunded: true,
          escrow: escrow ? serializeEscrow(escrow) : null,
